...
class MeshTensorDescriptor:
    ...
    def __init__(self, global_shape: Tuple[int, ...], mesh_layout: Tuple[int, int],
...
expected_print = """# from tvm.script import tir as T

@T.prim_func
def flash_attention(Q_handle: T.handle, K_handle: T.handle, V_handle: T.handle, Output_handle: T.handle):
    Q = T.match_buffer(Q_handle, (8, 4096, 4, 64), "bfloat16", strides=(1048576, 256, 64, 1))
    K = T.match_buffer(K_handle, (8, 4096, 4, 64), "bfloat16", strides=(1048576, 256, 64, 1))
    V = T.match_buffer(V_handle, (8, 4096, 4, 64), "bfloat16", strides=(1048576, 256, 64, 1))
    Output = T.match_buffer(Output_handle, (8, 4096, 4, 64), "bfloat16", strides=(1048576, 256, 64, 1))
    # with T.block("root"):
    ...
    with T.block("tilelang_root"):
        ...
        T.writes()
        Q_shared = T.alloc_buffer((64, 64), "bfloat16", scope="shared.dyn")
        K_shared = T.alloc_buffer((64, 64), "bfloat16", scope="shared.dyn")
        V_shared = T.alloc_buffer((64, 64), "bfloat16", scope="shared.dyn")
        acc_s = T.alloc_buffer((64, 64), scope="shared.dyn")
        acc_s_cast = T.alloc_buffer((64, 64), "bfloat16", scope="shared.dyn")
        acc_o = T.alloc_buffer((64, 64), scope="shared.dyn")
        scores_max = T.alloc_buffer((64,), scope="shared.dyn")
        scores_max_prev = T.alloc_buffer((64,), scope="shared.dyn")
        scores_scale = T.alloc_buffer((64,), scope="shared.dyn")
        scores_sum = T.alloc_buffer((64,), scope="shared.dyn")
        logsum = T.alloc_buffer((64,), scope="shared.dyn")
        acc_s_frag = T.alloc_buffer((64, 64), scope="local.fragment")
        scores_max_frag = T.alloc_buffer((64,), scope="local.fragment")
        acc_s_frag_1 = T.alloc_buffer((64, 64), scope="local.fragment")
        scores_sum_frag = T.alloc_buffer((64,), scope="local.fragment")
        T.dma_load(T.region(Q[bz, bx * 64, by, 0], 1, 1, 64, 1, 64), T.region(Q_shared[0, 0], 2, 64, 64), 0)
        T.fill(T.region(acc_o[0, 0], 2, 64, 64), 0)
        T.fill(T.region(logsum[0], 2, 64), 0)
        T.fill(T.region(scores_max[0], 2, 64), T.infinity("float32") * T.float32(-1.0))
        with T.LetStmt(((bx + 1) * 64 + 64 - 1) // 64, var=loop_range):
            for k in T.serial(loop_range, annotations={"num_stages": 3}):
                T.copy(T.region(K[bz, k * 64, by, 0], 1, 1, 64, 1, 64), T.region(K_shared[0, 0], 2, 64, 64), -1, T.bool(False), 0)
                for i in T.parallel(64):
                    for j in T.parallel(64):
                        acc_s[i, j] = T.if_then_else(bx * 64 + i >= k * 64 + j, T.float32(0.0), T.infinity("float32") * T.float32(-1.0))
                T.gemm_py(T.region(Q_shared[0, 0], 1, 64, 64), T.region(K_shared[0, 0], 1, 64, 64), T.region(acc_s[0, 0], 3, 64, 64), T.bool(False), T.bool(True), 64, 64, 64, 1, T.bool(False), 64, 64, 0, 0, 1, 0, T.uint32(0), 0, 0)
                T.copy(T.region(V[bz, k * 64, by, 0], 1, 1, 64, 1, 64), T.region(V_shared[0, 0], 2, 64, 64), -1, T.bool(False), 0)
                for i in T.parallel(64):
                    for j in T.parallel(64):
                        acc_s[i, j] = acc_s[i, j] * T.float32(0.18033688)
                T.copy(T.region(scores_max[0], 1, 64), T.region(scores_max_prev[0], 2, 64), -1, T.bool(False), 0)
                T.fill(T.region(scores_max[0], 2, 64), T.infinity("float32") * T.float32(-1.0))
                T.copy(T.region(acc_s[0, 0], 1, 64, 64), T.region(acc_s_frag[0, 0], 2, 64, 64), -1, T.bool(False), 0)
                T.reduce(acc_s_frag[0:64, 0:64], scores_max_frag[0:64], "max", 1, T.bool(False))
                T.copy(T.region(scores_max_frag[0], 1, 64), T.region(scores_max[0], 2, 64), -1, T.bool(False), 0)
                for i in T.parallel(64):
                    scores_max[i] = T.max(scores_max[i], scores_max_prev[i])
                for i in T.parallel(64):
                    scores_scale[i] = T.exp2(scores_max_prev[i] - scores_max[i])
                ...
                T.gemm_py(T.region(acc_s_cast[0, 0], 1, 64, 64), T.region(V_shared[0, 0], 1, 64, 64), T.region(acc_o[0, 0], 3, 64, 64), T.bool(False), T.bool(False), 64, 64, 64, 1, T.bool(False), 64, 64, 0, 0, 1, 0, T.uint32(0), 0, 0)
                T.copy(T.region(acc_s[0, 0], 1, 64, 64), T.region(acc_s_frag_1[0, 0], 2, 64, 64), -1, T.bool(False), 0)
                T.reduce(acc_s_frag_1[0:64, 0:64], scores_sum_frag[0:64], "sum", 1, T.bool(True))
                T.copy(T.region(scores_sum_frag[0], 1, 64), T.region(scores_sum[0], 2, 64), -1, T.bool(False), 0)
                for i in T.parallel(64):
                    logsum[i] = logsum[i] * scores_scale[i] + scores_sum[i]
            for i in T.parallel(64):
                for j in T.parallel(64):
                    acc_o[i, j] = acc_o[i, j] / logsum[i]
            T.dma_store(T.region(acc_o[0, 0], 1, 64, 64), T.region(Output[bz, bx * 64, by, 0], 2, 1, 64, 1, 64), 0)"""

flashattn.show()
print(flashattn.script() == expected_print)
